...
age_callout_queue(elapsed_time)
    int elapsed_time;
{//在过去了elapsed_time的时间后，执行应该发生的操作
    struct timeout_q *ptr, *expQ;
    
#ifdef CALLOUT_DEBUG
    IF_DEBUG(DEBUG_TIMEOUT)
		log(LOG_DEBUG,"aging queue (elapsed time %d):", elapsed_time);
    print_Q();
#endif
...
{//返回到一个计时器超时还有多久
    if (Q) {
				if (Q->time < 0) {
						log(LOG_WARNING,"timer_nextTimer top of queue says %d",Q->time);
						return 0;
				}
				return Q->time;
    }
...
{//设立一个计时器，参数分别为计时器的时间、超时后处理的函数名、函数所用到的参数
    struct     timeout_q  *ptr, *node, *prev;
    
#ifdef CALLOUT_DEBUG
    IF_DEBUG(DEBUG_TIMEOUT)
	log(LOG_DEBUG, "setting timer:");
    print_Q();
#endif
    ...
    if (node == 0) {
				log(LOG_WARNING,"Malloc Failed in timer_settimer\n");
				return -1;
    }
    node->func = action; 
    node->data = data;
...
{//用不到
    ...
    IF_DEBUG(DEBUG_TIMEOUT)
	for (ptr = Q; ptr; ptr = ptr->next)
	    log(LOG_DEBUG,"(%d,%d) ", ptr->id, ptr->time);
}
#endif /* CALLOUT_DEBUG */
