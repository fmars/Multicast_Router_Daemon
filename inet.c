 ...
 */
int
inet_valid_host(naddr)//是否是有效的主机地址
    u_int32 naddr;
{
 ...
 */
int
inet_valid_subnet(nsubnet, nmask)//是否是有效的子网地址
 ...
 */
char *
inet_fmt(addr, s)//将int型的IP地址转换成字符串型
    u_int32 addr;
    char *s;
{
    register u_char *a;
 ...
 */
int
inet_cksum(addr, len)//给地址和长度，计算校验和，下面的函数都用不到
	u_int16 *addr;
	u_int len;
{
